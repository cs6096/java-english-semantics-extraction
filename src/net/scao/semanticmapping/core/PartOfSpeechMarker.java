...
package net.scao.semanticmapping.core;
...
//			px(queryWord.substring(0, queryWord.length() - 1));
	...
	public static HashSet<EnglishTypeEnum> wordsPOS(String queryWord) throws Exception {
		HashSet<EnglishTypeEnum> fin = new HashSet<>();
		if (normalizedWord1(queryWord).equals("a")
				|| normalizedWord1(queryWord).equals("an")
				|| normalizedWord1(queryWord).equals("the")
				){
			fin.add(EnglishTypeEnum.ARTICLE);
			return fin;
		}
		if (isNoun(queryWord)){
			fin.add(EnglishTypeEnum.NOUN);
		}
		if (isVerb(queryWord)){
			fin.add(EnglishTypeEnum.VERB);
		}
		if (isAdjective(queryWord)){
			fin.add(EnglishTypeEnum.ADJECTIVE);
		}
		else if(isPronounDeterminer(queryWord)){
			fin.add(EnglishTypeEnum.POSSESSIVE_PRONOUN);
		}
		if (isAdverb(queryWord)){
			fin.add(EnglishTypeEnum.ADVERB);
		}
		if (isPreposition(queryWord)){
			fin.add(EnglishTypeEnum.PREPOSITION);
		}
		
		if (isToBeVerb(queryWord)){
			fin.add(EnglishTypeEnum.TO_BE_VERB);
		}
		if (isAuxiliary(queryWord)){
			fin.add(EnglishTypeEnum.AUXILIARY);
		}
		if (isPronoun(queryWord)){
			fin.add(EnglishTypeEnum.PRONOUN);
		}
		if (isWhPronoun(queryWord)){
			fin.add(EnglishTypeEnum.WH_PRONOUN);
		}
		if (isConjunction(queryWord)){
			fin.add(EnglishTypeEnum.CONJUNCTION);
		}

		if (normalizedWord1(queryWord).equals("there")){
			fin.add(EnglishTypeEnum.THERE);
		}

		if (fin.size() == 0){
			if (canBeProperNoun(queryWord)){
				fin.add(EnglishTypeEnum.PROPER_NOUN);
			}
		}
		...
		if (normalizedWord1(queryWord).equals("to")){
			fin.add(EnglishTypeEnum.TO);
		}
		if (queryWord.trim().equals(".")){
			fin.add(EnglishTypeEnum.FULL_STOP);
		}
		if (queryWord.trim().equals(",")){
			fin.add(EnglishTypeEnum.COMMA);
		}
		if (queryWord.trim().equals("?")){
			fin.add(EnglishTypeEnum.QUESTION_MARK);
		}
